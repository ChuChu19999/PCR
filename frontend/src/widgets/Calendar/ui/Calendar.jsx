...
const Calendar = ({ schedule }) => {
  const [isModalEventOpened, setModalEventOpen] = useState(false);
  ...
  const openAddModalWithButton = event => {
  ...
  const openAddModal = selectionInfo => {
    if (isModalEventOpened) {
      return;
    }
  ...
  const calendarConfig = useMemo(
    () => ({
      ...
      headerToolbar: {
        ...
        left: 'title',
        center: 'multiMonthYear,quarterGrid,dayGridMonth,timeGridWeek,timeGridDay',
        ...
        >
          <div className="form">
            ...
            <Form.Item
              ...
              rules={[{ required: true }]}
              className="form-item"
              initialValue={[dayjs(selectedEvent?.eventStart), dayjs(selectedEvent?.eventEnd)]}
            >
              <RangePicker showTime placeholder={['Выберите дату начала', 'Выберите дату конца']} />
            </Form.Item>

            <Form.Item layout="horizontal" name="color" label="Цвет события" className="form-item">
              <ColorPicker />
            </Form.Item>
          </div>
        </ModalForm>
